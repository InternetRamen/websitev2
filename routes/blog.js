...
router.post("/api", function (req, res, next) {
  ...
  if (!config.secrets.some((val) => val.key === bodyJson.secret))
    return res.sendStatus(401);
  if (data.body.some((val) => val.name === bodyJson.body.name))
    return res.sendStatus(304);
  bodyJson.body.date = Date.now();
  bodyJson.body.author = config.secrets.find(
    (val) => val.key === bodyJson.secret
  ).author;
  data.body.push(bodyJson.body);
  fs.writeFileSync(
    path.resolve(__dirname, "../public/data/posts.json"),
